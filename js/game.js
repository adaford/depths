...
export function loadSave() {
  try {
    ...
    if (!d || d.v !== 4 || !d.player) return false;
    resetG();
    Object.assign(G, d);
...
export function boot() {
  try {
    if (hasStore()) for (const k of ['depths_save_1', 'depths_save_2', 'depths_save_3']) localStorage.removeItem(k); // older schemas
  } catch (e) {}
