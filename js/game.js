...
export function loadSave() {
  try {
    ...
    if (!d || d.v !== 7 || !d.player) return false;
    resetG();
    Object.assign(G, d);
...
export function boot() {
  try {
    if (hasStore()) for (const k of ['depths_save_1', 'depths_save_2', 'depths_save_3', 'depths_save_4', 'depths_save_5', 'depths_save_6']) localStorage.removeItem(k); // older schemas
  } catch (e) {}
